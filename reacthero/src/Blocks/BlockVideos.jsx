...
const BlockVideos = () => {
    const { blockId,blockName } = useParams();
    ...
    const fetchPhotos = async () => {
        try {
            setLoading(true);
            const res = await fetch(`${API_BASE_URL}api/BlockVideo/GetVideoByBlockId/?id=${blockId}`);
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                const uploadRes = await axios.post(
                    `${API_BASE_URL}api/FileUpload/1/3`,
                    formData,
                    {
                ...
                await fetch(`${API_BASE_URL}api/BlockVideo`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${token}`,
                    },
                    body: JSON.stringify({ url: imageUrl, blockId: Number(blockId) }),
                });
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            for (const photoId of selectedPhotos) {
                const res = await fetch(`${API_BASE_URL}api/BlockVideo/${photoId}`, {
                    method: "DELETE",
                    headers: { Authorization: `Bearer ${token}` },
                });
    ...
    return (
        <Box sx={{ p: 3 }}>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3}>
                <Typography variant="h5">📸 ألبوم فيديو البلوك  {blockName}</Typography>
            ...
            {loading ? (
                ...
                <Typography color="text.secondary">لا توجد صور مرفوعة لهذا البلوك.</Typography>
                        ...
                        <Grid container spacing={2}>
                            {photos.map(p => {
                                const filePath = p.url?.replace(/\\/g, "/"); // لاحظ أنه عند رفع الفيديو نحفظه في p.url
                                const fullUrl = filePath?.startsWith("http") ? filePath : `${API_BASE_URL}${filePath}`;
                                const isVideo = /\.(mp4|webm|ogg)$/i.test(fullUrl); // تحقق إذا كان الفيديو
                                    ...
                                    <Grid item xs={6} sm={4} md={3} key={p.id}>
                                        ...
                                        >
                                            ...
                                            <Checkbox
                                                ...
                                                sx={{ position: "absolute", top: 8, left: 8, backgroundColor: "rgba(255,255,255,0.7)", borderRadius: "50%" }}
                                            />
                                            ...
                                            <Button
                                                ...
                                                color="error"
                                                variant="contained"
                                                sx={{ position: "absolute", top: 8, right: 8, minWidth: 0, borderRadius: "50%" }}
                                                onClick={() => {
